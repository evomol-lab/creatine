...
path <- paste(urld, "acc=GSE82291", "file=GSE82291_raw_counts_GRCh38.p13_NCBI.tsv.gz", sep="&");
...
gs <- factor(sml)
groups <- make.names(c("Control","Tumor"))
levels(gs) <- groups
...
plotDispEsts(ds, main="GSE82291 Dispersion Estimates")

# create histogram plot of p-values
hist(r$padj, breaks=seq(0, 1, length = 21), col = "grey", border = "white", 
         xlab = "", ylab = "", main = "GSE82291 Frequencies of padj-values")
...
par(mar=c(7,4,2,1))
boxplot(dat[,ord], boxwex=0.6, notch=T, main="GSE82291", ylab="lg(norm.counts)", outline=F, las=2, col=gs[ord])
