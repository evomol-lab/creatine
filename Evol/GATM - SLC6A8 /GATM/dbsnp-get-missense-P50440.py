...
# Mapeamento para código de 3 letras
AA_3LETTER = {
...
def obter_variacoes_dbsnp_gatm(uniprot_id="P50440"):
    """
    Obtém as variações missense do UniProt/dbSNP para a proteína GATM (P50440)
    e gera arquivos de saída com as mutações no formato AAposAA (ex: L2V, R3G).
    """
    url = f"https://www.ebi.ac.uk/proteins/api/variation/{uniprot_id}"
    headers = {"Accept": "application/json"}
    
    print(f"Buscando variações missense na UniProt API para {uniprot_id} (GATM)...")
    ...
    for f in features:
        ...
        if wild and mut and len(wild) == 1 and len(mut) == 1 and wild != mut:
            ...
            # Formato 1 letra: AAposAA (ex: L2V)
            aa_pos_aa = f"{wild}{pos}{mut}"
            
            # Formato 3 letras: AAposAA (ex: Leu2Val)
    ...
    print(f"Total de mutações missense encontradas para GATM: {len(df)}")
...
if __name__ == "__main__":
    obter_variacoes_dbsnp_gatm("P50440")
