...
counts_file = os.path.join(gatm_dir, "AM_classification_counts (2).tsv")

df_am = pd.read_csv(counts_file, sep="\t")

def parse_fasta_and_map_p50440(fasta_file):
    sequences = {}
    current_header = ""
    current_seq = []
...
# 1. Generate Individual Plots for each GATM FASTA file
for f in fasta_files:
    fname = os.path.basename(f)
    base_name = fname.replace(".fasta", "")
    pos_map, num_seqs = parse_fasta_and_map_p50440(f)
    ...
    # --- Subplot 3: Sequence Track along P50440 (Position vs Diversity & Pathogenic Count) ---
    ax3 = axes[2]
    ax3_twin = ax3.twinx()
    ...
    ax3.set_xlabel("P50440 Sequence Position", fontsize=12)
    ...
    plt.tight_layout()
    output_png = os.path.join(gatm_dir, f"{base_name}_diversity_vs_alphamissense.png")
    plt.savefig(output_png, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Saved individual plot: {output_png}")


# 2. Generate a 2x2 Grid Plot comparing ALL 4 GATM FASTA files side-by-side
...
fig.suptitle("AlphaMissense Pathogenicity vs. Alignment Residue Diversity across all GATM Alignments (P50440 Reference)", fontsize=16, fontweight='bold', y=1.06)

plt.tight_layout()
grid_output = os.path.join(gatm_dir, "GATM_all_alignments_diversity_vs_alphamissense.png")
